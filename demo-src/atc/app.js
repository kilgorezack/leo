/* ════════════════════════════════════════════════════════
   A YEAR OF EVENINGS — ATC edition

   365 squares, one per evening. The visitor picks a town
   and what their nights involve; we play the year out and
   count what a dish on the roof would have taken from them,
   then run the identical year on ATC's buried line.
...
(function(){
  ...
  /* severe-weather likelihood by month, per town */
  var REGIONS = [
    { k:'albion', label:'Albion',
      m:[.42,.38,.33,.22,.15,.09,.11,.12,.14,.24,.36,.44],
      winter:'snow', summer:'storm',
      note:'At the foot of the Albion Mountains, where the snow comes early and stays.' },
    { k:'arco',   label:'Arco',
      m:[.34,.32,.34,.30,.22,.14,.16,.18,.18,.24,.30,.36],
      winter:'wind', summer:'wind',
      note:'Out on the Lost River plain, where there is nothing to slow the wind down.' },
    { k:'malad',  label:'Malad City',
      m:[.38,.35,.30,.20,.14,.09,.10,.11,.13,.21,.32,.40],
      winter:'snow', summer:'storm',
      note:'Down in the valley, where winter settles in and does not move for a while.' },
    { k:'almo',   label:'Almo',
      m:[.36,.33,.29,.21,.16,.11,.15,.17,.15,.22,.31,.38],
      winter:'snow', summer:'storm',
      note:'Out past the Rocks, far enough that the weather is the least of it.' }
  ];
  ...
  var ACTS = [
    ...
    { k:'cams',   label:'Cameras out at the shop' },
  ...
  var LINES = {
    snow: [
      ...
      { act:'call',   t:'Snowing again. You finished the call on your phone, standing in the driveway.' },
      { act:'cams',   t:'Snow. The camera on the shop went offline and you found out Thursday.' },
    ...
    wind: [
      ...
      { act:'call',   t:'Wind all evening. Everyone kept asking if you were still there.' },
      { act:'movie',  t:'Wind across the flat. The picture came and went with it.' },
      { act:null,     t:'Wind all night, and the signal came and went with it.' }
    ],
    storm: [
      { act:'call',   t:'Thunderstorm came through at seven. You dropped off the call twice and gave up.' },
    ...
    peak: [
      ...
      { act:'call',   t:'Everybody in the valley sat down at once, and you were mid-sentence.' },
    ...
    block: [
      ...
      { act:null,     t:'Down for half an hour, and the help desk was two time zones away.' },
  ...
  var OURS_LINES = [
    ...
    'A fault down the road. Somebody was already headed out when it cleared.',
  ...
  var state = { region:'albion', acts:{call:true, movie:true}, mode:null, playing:false, ran:false };
  ...
  var CHECK='<svg viewBox="0 0 12 12" fill="none"><path d="M2 6.3 4.6 9 10 3.4" stroke="#FFFFFF" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  function simulate(){
    ...
    for(var i=0;i<DAYS;i++){
      ...
      if(rand() < region.m[month]){
        ...
        cause = (month <= 2 || month >= 10) ? region.winter : region.summer;
      }
  ...
  function finishDish(sim){
    ...
    elFlip.textContent = 'See the same year on ATC fiber';
  ...
  function finishOurs(sim){
    ...
    elA11y.textContent = 'With ATC fiber: no evenings lost across the year.';
  }
  ...
  function resetRun(){
    ...
    elFlip.textContent = 'See the same year on ATC fiber';
    elFlipText.textContent = 'Play the year, then switch the two above to run the exact same twelve months on a line that\'s buried between here and the office in Albion.';
    elPlay.disabled = false;
